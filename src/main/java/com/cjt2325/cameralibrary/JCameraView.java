...
public class JCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
    ...
    private void initView() {
        ...
        textViewCancel = (TextView) view.findViewById(R.id.textView_outside_cancel);
        setFlashRes();
...
//                machine.zoom(zoom, CameraInterface.TYPE_RECORDER);
            ...
            @Override
            public void recordOutSide(boolean isOutSide) {
                this.isOutSide = isOutSide;
                textViewCancel.setVisibility(isOutSide ? VISIBLE : GONE);
            }
    ...
    public void confirmState(int type) {
        switch (type) {
            case TYPE_VIDEO:
                ...
                machine.start(mVideoView.getHolder(), screenProp);
                if (jCameraLisenter != null) {
                    jCameraLisenter.recordSuccess(videoUrl, time);
                }
                break;
            case TYPE_PICTURE:
                mPhoto.setVisibility(INVISIBLE);
